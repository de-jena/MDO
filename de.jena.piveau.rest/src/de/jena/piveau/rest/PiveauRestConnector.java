...
//	@Reference
	...
	public Dataset createDataset(Dataset dataset, String datasetId, String catalogueId) {
		...
		Invocation invocation = target.path(config.cataloguesSegment()).path(catalogueId).path(config.datasetSegment())
				.path(config.originSegment()).queryParam("originalId", datasetId).request()
...
//				.header(REQUEST_BEARER_AUTH_HEADER, getJWTToken())
		...
		case NO_CONTENT:
			LOGGER.info(
					() -> String.format("Created data set with id '%s' for catalogue '%s' successfully with code %s",
							datasetId, catalogueId, type.getStatusCode()));
			return dataset;
		default:
			LOGGER.severe(() -> String.format("Error creating data set with id '%s' for catalogue '%s' with error %s",
					datasetId, catalogueId, type.getStatusCode()));
		}
		return null;
	}
...
//				.header(REQUEST_BEARER_AUTH_HEADER, getJWTToken())
		...
		case NO_CONTENT:
			LOGGER.info(
					() -> String.format("Deleted data set with id '%s' for catalogue '%s' successfully with code %s",
							datasetId, catalogueId, type.getStatusCode()));
			return true;
		default:
			LOGGER.severe(() -> String.format("Error deleting data set with id '%s' for catalogue '%s' with error %s",
					datasetId, catalogueId, type.getStatusCode()));
			return false;
		}
	}
	...
	public Distribution createDistribution(Distribution distribution, String datasetId) {
		...
		Invocation invocation = target.path(config.datasetSegment()).path(datasetId).path(config.distributionSegment())
				.request().header(REQUEST_AUTH_HEADER, getAPIKey())
//				.header(REQUEST_BEARER_AUTH_HEADER, getJWTToken())
		...
		case CREATED:
		case NO_CONTENT:
			LOGGER.info(String.format("Created distribution '%s' in dataset id '%s' successfully with code %s", id,
					datasetId, type.getStatusCode()));
			return distribution;
		default:
			LOGGER.severe(() -> String.format("Error distribution in data set with id '%s' with error %s", datasetId,
					type.getStatusCode()));
		}
		return null;
	}
...
//				.header(REQUEST_BEARER_AUTH_HEADER, getJWTToken())
		...
		case CREATED:
		case NO_CONTENT:
			LOGGER.info(() -> String.format("Deleted distribution with id '%s' successfully with code %s", id,
					type.getStatusCode()));
			return true;
		default:
			LOGGER.severe(() -> String.format("Error deleting distribution with id '%s' with error %s", id,
					type.getStatusCode()));
			return false;
		}
	}
...
//				.header(REQUEST_BEARER_AUTH_HEADER, getJWTToken())
		...
		case NO_CONTENT:
			LOGGER.info(
					() -> String.format("Indexed data set with id '%s' for catalogue '%s' successfully with code %s",
							datasetId, catalogueId, type.getStatusCode()));
			return true;
		default:
			LOGGER.severe(() -> String.format("Error indexing data set with id '%s' for catalogue '%s' with error %s",
					datasetId, catalogueId, type.getStatusCode()));
		}
		return false;
	}
