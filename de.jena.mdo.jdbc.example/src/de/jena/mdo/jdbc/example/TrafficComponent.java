...
/**
 * DB is 'traffic' TABLE is DETECTOR like the EClass name
 *
 * @author mark
 * @since 18.05.2022
 */
...
public class TrafficComponent implements DerbyDataImporter {
	...
	public Void run() {
		EMFPersistenceContext context = PersistenceHelper.createPersistenceContext(trafficBaseUri,
				trafficPackage.getDETECTOR(), null);
				...
				if (eo instanceof EPushStreamProvider) {
					...
					epsp.createPushStreamBuilder()
							...
							.withQueuePolicy(QueuePolicyOption.BLOCK).withBuffer(queue).build().onClose(() -> {
								System.out.println("Loading detectors: " + count.get() + " (" + (time.get() - start)
